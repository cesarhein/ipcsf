...
DaliPie = function (rawData, style, layout) {
	...
	var Y = {
		// Reference styles
		decFormat:{mode:"%", dp:1},
		weightFormat:{mode:"%", dp:2},
		changeFormat:{base:"weightFormat", forceSign:true},
		descriptionFormat:{base:"weightFormat", noSign:true},
		lines:{stroke:"#ddd"},
		axis:{
			maxNotches:6,
			label:{
		...
		getTooltip:function (A, D) {
			var S = this,
				asPercent = function (dec) {
					return (dec) ? zt.format(dec, Y.changeFormat) : "Sin cambios";
				};
			with (S.calcAll(A)) return "" +
				"<h3>" + category + "</h3>" +
				((A[2] > 0) ? "<b>" + asPercent(change) + "</b> en " + time : "") +
				(((time.substring(3, 7) + time.substring(0, 2)) >= (baseTime.substring(3, 7) + baseTime.substring(0, 2))) ? 
                                    "<br><b>" + asPercent(baseChange) + "</b> entre " + baseTime + " y " + time
                                    :
                                    "" ) +
				"<br><b>" + zt.format(baseWeight, Y.weightFormat) + "</b> de la canasta del IPC (" + baseTime + ")" +
				"<i>" + (
					(D.role == "bar") ?
						(!PAPER.isiPad) ?
							"<p>Haga clic para seleccionar este per&iacute;odo" :
							"<p>Haga clic nuevamente para seleccionar este per&iacute;odo" : // iPad
					(D.role == "slice" && childrenCategory.length) ?
						(!PAPER.isiPad) ?
							"<p>Ampliar" : // If it has children, then zoom must be possible
							"<p>Haga clic nuevamente para ampliar" : // If it has children, then zoom must be possible
					""
				) + "</p></i>";
		},
		setOffset:function () {
		...
		plan:{
			...
			title:{
				...
				curr:function (S, D, mode) {
					...
					return {
						...
						subtitle:{
							text:"desde " + baseTime + " hasta " + currTime
						}
					};
				}
			},
			slice:{
 			...
 			zoomOutTab:{
				...
				init:function (S, D, mode) {
					return {
						text:"Alejar", layout:L.zoomOutTab, layer:2,
						mouseEvents:function (D) {
			...
			text:{
				...
				curr:function (S, D, mode) {
					...
					with (data) return {content:
						"<h2>" + category + "</h2><p>" +
						category + (
							(childrenCategory.length) ? " contiene " +childrenCategory.length + " subcategor&iacute;as" :
							" es una subcategor&iacute;a de " + parentCategory) +
						" y fu&eacute; " + zt.format(baseWeight, Y.weightFormat) + " de la canasta del IPC " + baseTime + ".<p>" +
						"Los precios " + (
							(!change) ? "no cambiaron" :
							((change < 0) ? "bajaron " : "subieron ") + zt.format(change, Y.descriptionFormat)
						) + " en " + time + ", " +
						"y fueron " + zt.format(baseChange, Y.descriptionFormat) + ((baseChange < 0) ? " inferiores" : " superiores") +
						" a lo que eran en " + baseTime + "."
					};
				}
			},
			bar:{
