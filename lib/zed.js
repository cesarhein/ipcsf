...
var zPlot = zp = {
	...
	degDiff:function (a, b) {return zt.round(Math.abs(zp.matchDeg(a) - zp.matchDeg(b)), 10)},
	////////////////////
	//  Vector tools  //
	////////////////////
...
var zObject = zo = {
	...
	equals:function (a, b, ordered) {
		...
		if (type == "number") {
			return zt.round(a, 15) == zt.round(b, 15); // Floats don't quite add up sometimes - if it's accurate to 15 decimal places, it's close enough
		};
...
var zTools = zt = {
	...
	*/
	...
	makePaper:function (layout) {
		...
		paper.isiPad = navigator.userAgent.match(/iPad/i) != null;
	...
	getMaxLayout:function (minWidth, minHeight, margin) {
		return new zLayout({ // Layout for the whole canvas
			...
			height:Math.max(minHeight, $(window).height() - $("#Raphael").offset().top - 16), // Fit to screen, with minimum size
			margin:margin
		});
	},
...
function zLayout (layout, parent) {
	...
	c.set = function (layout) {
		if (!layout) return c;
		c = zo.extend(c, (layout instanceof zLayout) ? layout.getVitals() : layout); // Load whatever is in layout (if it's an object, only load the vitals, not the object itself), then recalculate all derived information
		// Anchor
		...
		} else if (layout.width == null) { // If width is not defined, calculate width from innerWidth (default = 0 + 2 * margin)
		...
		} else if (layout.height == null) { // If height is not defined, calculate height from innerHeight (default = 0 + 2 * margin)
		...
		if (c.radial == null) { // Normal alignment
			...
			var normRadial = zp.matchDeg(c.radial); // Normalise so that quadrants can be calculated, but keep original so spinning works properly
	...
	c.clone = function (layout) {
		return new zLayout(c.getVitals(layout));
	};
	// Return the minimum set of values required to fully reconstruct this layout
	c.getVitals = function (layout) {
		var i, out = {},
...
*/
function zDataCube (dLen, rawData) {
	var c = this;
	///////////////
	//  Parsing  //
	///////////////
	...
	c.getMeta = function (type, x) {
		...
		if (type == "all") {
			...
			if (!meta[type]) return logger("zDataCube.getMeta(): Metadata type " + type + " in dimension " + x.d + " is empty or does not exist.");
	...
	c.setColour = function (x, palette, mode) {
		...
		if (mode == "tree" || mode == "randomTree") {
			if (palette.length < 3) explode("zDataCube.setColour(): Can't do a tree assignment with less than 3 colours.");
			g = c.meta[x.d].tree;
	...
	c.getVal = function (s) {return za.getDeep(c.data["val"], c.asASpace(s))}; // Shortcut for getData("val")
	// Apply za.calc functions (sum, mean, etc.)
		...
		if (d != null) {
			...
			if (parentName) for (parent = [], i = 0; i < c.getSize(d); i++) {
				parent[i] =
					...
					c.shyPushMeta("name", d, parentName[i]); // Extend is a shy function - if the name already exists, it will only find rather than extend
			};
	...
	c.addRootNode = function (d, name) {
		...
		c.setMeta(null, {d:d, a:aPos}, {
			...
			generation:0, leaves:c.calcMeta("sum", "leaves", {d:d, a:meta.tree[0]})
		});
		...
		c.forSpaces({as:"all", mask:{d:d, a:"na"}}, function (space) { // Iterate through each space NOT in the selected dimension
			if (mode == "stacked") { // For stacked, calculate children based on parent value
				...
				c.forGenerations(d, function (x) { // For each generation in the selected dimension
					c.forData(type + "Stacked", {as:c.addSpaces(space, x)}, function (oldVal, x) { // Go through each member
						// TODO: Does not check if children are valid
						x.mask = {d:d, a:c.findChildren({d:d, as:x.as})}; // Find its children
				...
				c.forGenerations(d, function (x) { // For each generation in the selected dimension
					c.forData(type, {as:c.addSpaces(space, x)}, function (oldVal, x) { // Go through each member
...
// 		c.verifyTreeData(mode, type, d);
		...
		c.forSpaces({as:"all", mask:{d:d, a:"na"}}, function (space) { // Iterate through each space NOT in the selected dimension
			if (mode == "stacked") { // For stacked, calculate children based on parent value
				c.forGenerations(d, function (x) { // For each generation in the selected dimension
					c.forData(type + "Stacked", {as:c.addSpaces(space, x)}, function (oldVal, x) { // Go through each member
						space.as[d] = c.findChildren({d:d, as:x.as});
						if (!space.as[d].length) logger("zDataCube.calcTreeData(): Hmm - " + zt.asString(c.getName({d:d, as:x.as})) + " is in generation " + c.getMeta("generation", {d:d, as:x.as}) + " of " + c.meta[d].tree.length + ", but it has no children. Just sayin'.");
						else {
							var newVal = za.mergeStacked(c.getData(type + "Stacked", space));
							if (!zo.equals(newVal, oldVal)) logger("zDataCube.calcTreeData(): UH OH - Range for node " + zt.asString(c.getName({d:d, as:x.as})) + " is " + zt.asString(oldVal) + ", but its children is " + zt.asString(newVal) + " (type:" + type + ").");
						};
						return oldVal; // .forData() is expecting a value
					});
				}, false, 0, 1); // Top to bottom, skip bottom-most generation
	...
	c.setTreemap = function (x, vals, layout) {
	...
	//  Push functions  //
	//////////////////////
	// Override names with displayNames - only to be used on initialisation
	c.setDisplayName = function (d) {
		if (d != null) {
			if (!c.meta[d].displayName) return; // No display name, quit
			c.forMeta("name", {d:d, a:"all"}, function (oldVal, x) {
				return c.meta[d].displayName[x.a] || oldVal;
			});
		} else for (d = 0; d < c.dLen; d++) c.setDisplayName(d);
	};
	...
	c.importProtoCube = function (protoCube) {
		...
		c.setShown(); // Recalculates dimension sizes, based on the new names
		c.makeTree(); // Construct trees if required
		c.setDisplayName(); // Replace name with displayName
		return c;
	};
	// Initialise
...
function zSimpleDrone (c, orders, type) {
	zo.shyExtend(c, {
		...
		setMode:function (mode, t, e, b, w) {
			...
			c.O = c.orders = zo.extend(c.O, c.parseOrders(mode));
			c.reset(t, e, b, w);
		},
		unsetMode:function (t, e, b, w) {
			if (!c.origO) return;
			c.O = c.orders = c.origO;
			c.origO = null;
			c.reset(t, e, b, w);
		},
		...
		startMouseEvents:function () {
			if (!c.O || !c.O.mouseEvents) return; // No mouseEvents defined
			c.stopMouseEvents();
			c.O.mouseEvents(c);
		},
		...
		tooltip:function (tooltipText, tooltipBox, layout) {
			...
			c.hover(function (event) {
				...
				tooltipBox.moveToEvent(event, layout);
			}, function () {tooltipBox.hide()}); // Instant hide avoids flickering and removes problems on iPad
		}
	});
	// Initialise
	if (orders.base) orders = zo.clone(zo.remove(orders, "base"), orders);
	if (c.setParts) c.setParts(orders);
	c.O = c.orders = c.parseOrders(orders);
	var layout = orders.L || orders.layout;
	if (layout) {
		if (orders.margin != null) layout.margin = orders.margin; // O.margin overrides layout.margin
		c.L = c.layout = new zLayout(layout, c);
	};
};
...
function zGuide () {
	var c = this;
	c.redraw = function (t, e, b, w) {
		c.redrawTo(0); // Reset position first
		c.redrawTo(1, t, e, b, w);
	};
	c.redrawTo = function (val, t, e, b, w) {
		if (b) c.bQueue.push(b); // Any b that's asked for will be added to the queue REGARDLESS OF WHETHER ANIMATION STARTS
		if (c.scripting) return; // A scripting redraw is already running - don't change the state of the guide
		c.stop();
		c.animating = true;
		c.animate({path:zp.start({x:val * 1000, y:0})}, t, e, function () {
			var i, q = c.bQueue; // Save and reset queue before calling any other function (which might use this guide)
			c.bQueue = [];
			c.animating = false;
			for (i = 0; i < q.length; i++) {
				if (typeof q[i] == "function") q[i]();
				else logger("zGuide.redrawTo(): WHOOPS - " + zt.asString(q[i]) + " has been presented as a function.");
			};
		}, w);
	};
	c.stop = function () {
	...
	c.runScript = function (script, w) {
		...
		if (!PAPER.noAnimation) { // Animate
			...
			c.redrawTo(0); // Must be set to 0 so getVal() will work correctly
			c.redrawTo(1, t, e, function () {c.runScript(script, w)}, w); // Run again when finished, but only if there's more script left to run
			c.scripting = true; // Scripting flag will stop guide from drawing again - the scripting redraw should be the ONLY redraw
			if (action) action(t, e, c.K); // All the callback actions will be logged first, before the script to run is added
			c.scripting = false;
		} else for (var i = 0; i < script.length; i++) script[i][1](); // No animation
	};
	c.delay = function (t, action) {
		c.redraw(t, "-", action);
	};
	// Initialise
	zSimpleDrone(c, {}, "zGuide");
	c.K = c.add(PAPER.path(zp.start()));
	c.animating = false;
	c.bQueue = [];
};
...
function zLine (orders, t, e, b, w) {
	...
	c.redraw = function (orders, t, e, b, w) {
		...
		c.animate(orders, t, e, b, w);
	};
	// Initialise
	zSimpleDrone(c, orders, "zLine");
	c.add(PAPER.path(zp.line(c.O.points)), c.O);
	c.appear(t, e, b, w);
};
...
function zShape (orders, t, e, b, w) {
	...
	// Initialise
	zSimpleDrone(c, orders, "zShape");
	c.add(PAPER.path(zp.shape(c.O.points)), c.O);
	c.appear(t, e, b, w);
};
...
function zArc (orders, t, e, b, w) {
	...
	// Initialise
	zSimpleDrone(c, orders, "zArc");
	c.add(PAPER.path(zp.arc(c.O.arc)), c.O);
	c.appear(t, e, b, w);
};
...
function zCircle (orders, t, e, b, w) {
	...
	// Initialise
	zSimpleDrone(c, orders, "zCircle");
	var a = c.O.circle;
	c.add(PAPER.circle(a.centre.x, a.centre.y, a.radius), c.O);
	c.appear(t, e, b, w);
};
...
function zRectangle (orders, t, e, b, w) {
	var c = this;
	zSimpleDrone(c, orders, "zRectangle");
	// Choose mode (mode is fixed upon creation)
	if (!c.L && c.O.rect) { // No layout, direct draw based on rect attribute (fastest)
		c.redraw = function (orders, t, e, b, w) {
			if (!orders) return;
		...
		var r = c.O.rect;
		c.add(PAPER.rect(r.x, r.y, Math.abs(r.width), Math.abs(r.height)), c.O);
	} else if (c.O.rounded == null) { // Rect mode, with layout (marginally slower)
		c.redraw = function (orders, t, e, b, w) {
			if (!orders) return;
			c.L.set(orders.layout);
			if (orders.layout) zo.extend(orders, c.L.getRect());
			c.animate(orders, t, e, b, w);
		};
		var r = c.L.getRect();
		c.add(PAPER.rect(r.x, r.y, r.width, r.height), c.O);
		...
		c.getSVG = function (rounded) {
			if (!rounded || !c.L.width || !c.L.height) return zp.shape(c.L.corners);
			var direction = 1;
			if (c.L.xAlign == "right") direction *= -1;
			if (c.L.yAlign == "bottom") direction *= -1;
			if (c.L.width < 0) direction *= -1;
			if (c.L.height < 0) direction *= -1;
			var start = zp.start, line = zp.lineTo,
				arc = (direction == 1) ? zp.arcTo : zp.reverseArcTo,
				xDec = rounded / Math.abs(c.L.width),
				yDec = rounded / Math.abs(c.L.height);
			return (
				start(c.L.getPoint(xDec, 0)) +
				line(c.L.getPoint(1 - xDec, 0)) +
				arc(rounded, c.L.getPoint(1, yDec)) +
				line(c.L.getPoint(1, 1 - yDec)) +
				arc(rounded, c.L.getPoint(1 - xDec, 1)) +
				line(c.L.getPoint(xDec, 1)) +
				arc(rounded, c.L.getPoint(0, 1 - yDec)) +
				line(c.L.getPoint(0, yDec)) +
				arc(rounded, c.L.getPoint(xDec, 0)));
		};
		c.redraw = function (orders, t, e, b, w) {
			if (!orders) return;
			c.L.set(orders.layout);
			if (orders.rounded != null) orders.path = c.getSVG(orders.rounded); // Rounding has changed (Path must be entirely recalculated either way)
			else if (orders.layout) orders.path = c.getSVG(c.O.rounded); // Layout has changed (Path must be entirely recalculated either way)
			c.animate(orders, t, e, b, w);
		};
		var svg = c.getSVG(c.O.rounded);
		c.add(PAPER.path(svg), c.O); // Rounded must be set from the start
	};
	c.appear(t, e, b, w);
};
...
function zText (orders, t, e, b, w) {
	...
	c.parseText = function (text) {
		var f = c.O.format;
		if (!f || typeof f != "object" || text == "") return text;
	...
	c.redraw = function (orders, t, e, b, w) {
		if (!orders) return;
		var textXAlign, textYAlign, boxX, boxY, textX, textY;
		orders = c.parseOrders(orders); // Parse new style
		if (orders.text != null) orders.text = c.parseText(orders.text); // Parse text
		// Calculate new layout
		c.getBBox(orders); // Apply styles and calc new bBox
		c.L.set(orders.layout); // Apply new layout
		// Calculate text position (relative to layout)
		textXAlign = orders.textXAlign || c.O.textXAlign || c.L.xAlign; // If no textAlignment is defined, use layout alignment
		textYAlign = orders.textYAlign || c.O.textYAlign || c.L.yAlign;
		...
		c.animate(orders, t, e, b, w); // Redraw
	};
	// Initialise
...
function zImage (orders, t, e, b, w) {
	...
	c.redraw = function (orders, t, e, b, w) {
		c.L.set(orders.layout);
		zo.extend(orders, {
			x:Math.round(c.L.left + c.L.margin), y:Math.round(c.L.top + c.L.margin),
			width:Math.round(c.L.innerWidth), height:Math.round(c.L.innerHeight)
		});
		c.animate(orders, t, e, b, w);
	};
	// Initialise
	zSimpleDrone(c, orders, "zImage");
	c.add(PAPER.image(c.O.url, c.L.left, c.L.top, c.L.innerWidth, c.L.innerHeight), c.O);
	c.appear(t, e, b, w);
};
...
function zSVG (orders, t, e, b, w) {
	...
	c.redraw = function (orders, t, e, b, w) {
		c.animate(orders, t, e, b, w);
		if (c.L && orders.layout) {
			var prevAnchor = {x:c.L.x, y:c.L.y};
			c.L.set(orders.layout);
			c.raphObj.translate(c.L.x - prevAnchor.x, c.L.y - prevAnchor.y);
		};
	};
	// Initialise
	zSimpleDrone(c, orders, "zSVG");
	c.add(PAPER.path(c.O.path), c.O);
	c.appear(t, e, b, w);
};
...
function zComplexDrone (c, orders, type) {
	zSimpleDrone(zo.shyExtend(c, {
		...
		show:function (t, e, b, w) {
			c.invisibleShow();
			var p, orders = {opacity:c.O.opacity || 1};
			for (p in c.parts) if (c.O[p] && c.O[p].opacity) orders[p] = {opacity:c.O[p].opacity}
			c.animate(orders, t, e, b, w);
		},
		// Show object in the dom, but don't change its opacity
		invisibleShow:function () {
		...
		//  Events  //
		//////////////
		stopMouseEvents:function () {c.forEach("stopMouseEvents")},
		setParts:function (orders) {
			for (var p in orders) {
				if (orders[p] && orders[p].type) {
					c.parts[p] = {}; // Create placeholders for parts so parseOrders can work properly
				};
			};
		}
	}), orders, type);
...
function zTextBox (orders, t, e, b, w) {
	...
	c.L = c.layout = c.parts.textObj.L;
...
function zTooltipBox (orders) {
	...
	c.type = "zTooltipBox";
	c.hide();
	c.hover(function () {c.show(100)}, function () {c.hide()});
	c.click(function () {c.hide()});
	return c;
};
...
function zHTML (orders, t, e, b, w) {
	...
	c.add = function (orders) {
		var html = $("<div" + ((orders.objClass) ? " class=" + orders.objClass : "") + ((orders.objID) ? " id=" + orders.objID : "") + "></div>"); // Don't change name to class - it breaks it in goddamn retarded IE
		c.raphObj = html;
 		$("#Raphael").append(html);
		return html;
	};
	...
	c.stopMouseEvents = function () {$(c.raphObj).unbind()}; // Use jQuery
...
function zNarrator (orders) {
	var c = new zHTML(zo.extend({
		objID:"narrator",
		base:{ // Rest of style is CSS defined
...
Plan
	...
	Each subplan consists of these elements:
		type: A zDrone class.
		mask: Swarm combines swarm.aSpace and swarm.plan.[ELEMENT].mask to create a list, containing the aSpace of every drone of that type which should be created.
...
Filters
	...
	Mask: A valid aSpace mask. (e.g. ["mask", 3, "mask"], or "all" or "mask" (== ["mask", "mask", "mask"] == c.aSpace)
		"all" refers to every drone of every type (as defined in plan) for every single aSpace. This is the default.
		"curr" calls the drones grabbed by setCurrDrones()
		"fixed" is a special mask, which does not correspond to an aSpace
...
function zSwarmConstructor (c, orders, type) {
	zo.shyExtend(c, {
		...
		lock:function () {
			if (c.locked != "Superlocked") {
				c.locked = Math.round(Math.random() * 9000) + 999; // Cannot override superlock
				c.stopMouseEvents("all");
			};
		},
		unlock:function () {
			if (c.locked != "Superlocked") {
				c.locked = null;
				c.startMouseEvents("all");
			};
		},
		...
		//  Magic sauce  //
		///////////////////
		// Change the aSpace for the swarm and all drones
		setSpace:function (x, t, e, b, w) {
			var d = (x.d == null) ? c.d : x.d,
				aPos = c.asAPos(x);
			if (aPos == null) return logger(c.type + ".setSpace(): No aPos specified (" + zt.asString(x) + ").");
			if (c.A == "fixed") return logger(c.type + ".setSpace(): Can't change " + zt.asString(c.A) + " to " + zt.asString(aPos) + ".")
			if (typeof c.A[d] != typeof aPos) return; //logger(c.type + ".setSpace(): Can't change " + zt.asString(c.A[d]) + " to " + zt.asString(aPos) + " (different types).");
			c.A[d] = zo.clone(aPos); // Change aSpace for swarm
			c.forRoles(function (role) {
				if (role.mask == "mask" || role.mask[d] == "mask") { // Change aSpace for drones which are valid candidates
					c.forDrones(role.name, function (D) {D.A[d] = zo.clone(aPos)});
				};
			});
		},
		...
		get:function (x) {
			...
			for (d = 0; d < A.length; d++) { // Use only active parts of the mask as filtering conditions
				if (out.length == 0) break; // If there are no more candidates, quit
				if (typeof A[d] == "number") {
					out = za.getAllObjects(out, "A", A[d], "atPos", d); // atPos is much more efficient than matching whole addresses
				} else if (za.isArray(A[d])) {
					matches = [];
					for (i = 0; i < out.length; i++) {
						if (za.contains(A[d], out[i].A[d])) matches.push(out[i]);
					};
					out = matches;
		...
		plot:function (x, forced) {
			...
			for (i = 0; i < drones.length; i++) { // Can't use forDrones, as it must go forwards not backwards
				...
				D.newOrders = (c.P[D.role].curr) ? c.P[D.role].curr(c, D, mode, D.A, c.Y[D.role]) : {};
		...
		addDrone:function (role, A, mode, t, e, b, w) {
			var orders, plan = c.P[role],
				D = {
					id:zt.asString(A) + " " + role,
					A:A,
					O:{}, // Placeholder for swarms serving as drones, as drones are expected to have .O, but swarms do not
					Y:zo.clone(c.Y[role]),
					role:role,
					parent:c,
					directRedraw:plan.directRedraw // Set this flag to avoid frameRedraw and directly redraw using Raphael
				};
			if (za.getObject(c.drones, "id", D.id)) return;// logger(c.type + ".addDrone(): Uh oh - A " + role + " at " + zt.asString(A) + " already exists."); // Check if it already exists
			// Parse orders
			D.currOrders = (plan.curr) ? plan.curr(c, D, mode, D.A, D.Y) || {} : {};
			orders = zo.clone(c.Y[role]); // Start with style
			orders = zo.extend(orders, D.currOrders); // Add currOrders
			orders = zo.extend(orders, (plan.init) ? plan.init(c, D, mode, D.A, D.Y) || {} : {}); // Add initOrders
			// Create drone from orders
			if (orders.ignore) return;
			else if (typeof plan.type == "string" || typeof plan.type == "function") {
				D = zo.extend(zDrones.make(plan.type, orders, t, e, null, w), D);
				c.drones.push(D);
				if (D.class == "zSwarm") {
					if (D.onAdd) D.onAdd(D);
				} else {
		...
		addRole:function (role, A, mode, t, e, b, w) {
			if (role.mask == "fixed") c.addDrone(role.name, "fixed", mode, t, e, null, w);
			else {
				var i, spaces = c.dc.listSpaces({as:A, mask:{as:role.mask}});
				for (i = 0; i < spaces.length; i++) {
					c.addDrone(role.name, spaces[i], mode, t, e, null, w);
				};
			};
		},
		...
		add:function (x, t, e, b, w) {
			var role = c.getRole(x),
				...
				A = c.dc.addSpaces(c.A, (typeof x == "string") ? null : c.asASpace(x));
			mode.initialising = true;
			if (role) { // A role is specified
				role.ignore = false; // Cancel ignore flag
				c.addRole(role, A, mode, t, e, null, w || c.K);
			} else if (x == "fixed") { // Fixed
				c.forRoles(function (role) {
					if (role.mask == "fixed") c.addRole(role, A, mode, t, e, null, w || c.K);
				});
...
// 				if (t && c.frame) logger(c.type + ".redraw(): " + c.frame + " frames in " + t + " milliseconds."); // Count frames
		...
		//  Events  //
		//////////////
		startMouseEvents:function (x) {c.forDrones(x, "startMouseEvents")},
		stopMouseEvents:function (x) {c.forDrones(x, "stopMouseEvents")},
	...
	c.highlighted = [];
	// Aliases
	c.baseHighlight = c.highlight;
	c.baseReset = c.reset;
	c.baseSelect = c.select;
	c.baseDeselect = c.deselect;
	// Parse orders
	...
	// Parse space
	c.A = c.aSpace = c.A || c.aSpace || "all";
	if (c.dc) c.A = c.aSpace = c.dc.asASpace(c.A);
	// Parse style
	var style = zo.clone(c.style || {}, c.Y);
	c.Y = c.style = zo.parseStyle(style);
	// Parse layout
	var layout = c.L || c.layout || c.Y.layout || {};
	if (c.style.margin != null) layout.margin = c.style.margin; // Style override of margins
	c.L = c.layout = new zLayout(layout); // Make new layout object if the original is not a zLayout object
	// Parse plans
	for (p in c.plan) {
		if (c.plan[p]) {
		...
		plan:{
			...
			axisTitle:{
				...
				init:function (S, D, mode) {
					var reverse = zt.isBetween(S.L.rotation, 89, 270, true); // Upside-down
					return {
						text:S.title || S.Y.title || S.dc.getMeta("title", {d:S.d}),
						layout:{
							anchor:S.L.getPoint(0.5, 1),
							xAlign:"xCentre",
							yAlign:(reverse) ? zp.inverseAlignment(S.L.yAlign) : S.L.yAlign,
							rotation:(S.L.rotation || 0) + ((reverse) ? 180 : 0)
						}
					};
				}
			},
			label:{
				...
				init:function (S, D, mode) {
					if (S.trimmedShown && !za.contains(S.trimmedShown, D.A[S.d])) return {ignore:true};
					return {
						layer:1,
						text:S.dc.getName({d:S.d, a:D.A[S.d]}),
				...
				curr:function (S, D, mode) {
					return {layout:S.getPoint({a:D.A[S.d]})};
				}
			}
		}
	}, o), "zUndefinedAxis");
	// Initialise
	if (!c.A || !(c.A instanceof Array)) c.A = c.aSpace = []; // Invalid aSpace
	if (c.A[c.d] == null) c.A[c.d] = "all";
	for (var d = 0; d < c.dc.dLen; d++) {
		if (d != c.d) c.A[d] = "na";
	};
	if (c.dc) c.A = c.aSpace = c.dc.asASpace(c.A);
...
function zLegend (orders, t, e, b, w) {
	return new zSmartAxis(zo.extend({
		...
		plan:{
			notch:{
				init:function (S, D, mode) {
					return {fill:S.dc.getMeta("colour", {d:S.d, a:D.A[S.d]})};
				}
			}
		}
	}, orders), t, e, b, w);
		...
		plan:{
			...
			upArrow:{
				...
				curr:function (S, D, mode) {
					...
					return {
						points:[
							{x:l.x, y:l.top - D.Y.xSize},
							{x:l.x - D.Y.ySize / 2, y:l.top},
							{x:l.x + D.Y.ySize / 2, y:l.top}
						]
					};
				}
			...
			downArrow:{
				...
				curr:function (S, D, mode) {
					...
					return {
						points:[
							{x:l.x, y:l.bottom + D.Y.xSize},
							{x:l.x - D.Y.ySize / 2, y:l.bottom},
							{x:l.x + D.Y.ySize / 2, y:l.bottom}
						]
					};
				}
...
function zSlideAxis (orders, t, e, b, w) {
	return new zSmartAxis(zo.extend({
		...
		baseSelected:{r:0},
		style:{
			disableBackwards:false, // When on, forces selected to always be >= baseSelected
			perpendicularSize:1,
			select:{t:200, e:"-"},
			slideBar:{
				fill:"#801c7e",
		...
		setBase:function (x, t, e, b, w) {
			var c = this;
			c.baseSelected = {d:c.d, a:c.asAPos(x), r:c.asRPos(x)};
			if (c.style.disableBackwards && c.baseSelected.r > c.selected.r) {
				c.axisSelect(c.baseSelected, t, e, b, w);
			} else {
				c.forSwarms("refresh", "all", t, e, b, w); // Redraw all swarms
			};
		},
		axisSelect:function (x, t, e, b, w) {
			var c = this, thread = Math.round(Math.random() * 1000), // Current thread
				rPos = Math.round(zt.forceBetween((zp.isPoint(x)) ? c.pointToRPos(x) : c.asRPos(x), c.range[0], c.range[1])),
				plotSelected = function (rPos, forced) {
					...
					if (c.style.disableBackwards && c.baseSelected.r > c.selected.r) {
						c.baseSelected = zo.clone(c.selected);
					};
					c.G.scripting = false;
			...
			if (!c.G.animating) { // New animation
				...
				c.forOwnedSwarm("stopMouseEvents");
			...
			c.G.redrawTo(c.rTargSelected, t, e, function () { // Animate, and when complete..
				...
				c.forOwnedSwarm("startMouseEvents");
				if (b) b();
				c.G.animating = false;
			}, w);
		},
			...
			slideBar:{
				...
				init:function (S, D, mode) {
					...
					return {
						...
						background:{
							type:"zRectangle",
							layout:{
								x:S.getX({r:0}) - (D.Y.rounded || 0),
								y:S.getY({r:0}),
								height:S.notchHeight,
								width:S.getLength(S.range[1]) + 2 * (D.Y.rounded || 0)
							}
						},
						foreground:{
							type:"zRectangle",
				...
				curr:function (S, D, mode) {
					return {
						foreground:{
							layout:{
								x:S.getX(S.baseSelected) - (D.Y.rounded || 0),
								y:S.getY(S.baseSelected),
								height:S.notchHeight,
								width:S.getLength(S.selected.r - S.baseSelected.r) + 2 * (D.Y.rounded || 0)
							}
						}
					};
				}
			...
			baseNotch:{
				...
				curr:function (S, D, mode) {
					var x = {d:S.d, a:S.baseSelected.a},
						...
						flag = zp.flag({
							anchor:anchor,
							width:D.Y.width, height:D.Y.height,
							xAlign:"right", yAlign:"bottom"
						}, D.Y.flagWidth, D.Y.flagHeight),
						labelLayout = {anchor:zo.mid(flag[0], flag[2]), xAlign:"xCentre", yAlign:"yCentre"};
					return {
						ring:{
							circle:{
								centre:anchor,
								radius:(1 + S.L.height - D.Y.ring["stroke-width"]) / 2
							}
						},
...
// 							type:"zRectangle", layout:layout, // Square button
						...
						mouseEvents:function (D) {
							D.click(function () {
								if (S.G.animating) S.axisSelect(S.selected, sa.t, sa.e); // Stop
...
function zSwarm (orders, t, e, b, w) {
	var c = this, p;
	zSwarmConstructor(c, zo.extend({A:"all"}, orders), "zSwarm");
	// Initialise
	if (c.updateData) c.updateData(t, e, null, c.K);
	c.add("fixed", t, e, null, c.K);
	c.add("all", t, e, b, w); // Add everything
	c.layer();
	for (p in c.axes) c.axes[p].layer(); // Relayer axes, in case swarm covered them
};
