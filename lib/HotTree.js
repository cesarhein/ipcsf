HotTree = function (rawData, style, layout) {
	...
	// Layout
	L = zt.getMaxLayout(780, 600, 40);
	L.set(layout); // Overwrite layout with user-defined layout
	with (L) {
		L.tooltipBox = {
			width:240
		};
...
// 			width:400,
	...
	with (L.sideBar) {
		...
		L.scale = new zLayout({
			x:left + margin + innerWidth * 0.1, y:L.title.y + 44,
			width:innerWidth * 0.7, height:innerHeight * 0.012
		});
		L.description = {
